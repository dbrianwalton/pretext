...
<!-- Dynamic: Generate a discrete random parameter. -->
<!-- @min: lowest value -->
<!-- @max: highest value -->
...
<xsl:template name="random-parameter-discrete">
  <xsl:param name="mode">webwork</xsl:param>
  ...
  <xsl:if test="$mode = 'runestone'">
    <xsl:text>v.rng.</xsl:text>
  </xsl:if>
  <xsl:choose>
...
<!-- Dynamic: Generate a uniform random parameter. -->
<!-- @min: lowest value -->
<!-- @max: highest value -->
<xsl:template name="random-parameter-uniform">
  ...
  <xsl:choose>
    <xsl:when test="$mode = 'runestone'" >
      <xsl:text>v.rng.runif(</xsl:text>
      <xsl:value-of select="$param-rnd-min"/>
      <xsl:text>,</xsl:text>
      <xsl:value-of select="$param-rnd-max"/>
      <xsl:text>)</xsl:text>
    </xsl:when>
    <xsl:when test="$mode = 'webwork'" >
      <xsl:text>(</xsl:text>
      <xsl:value-of select="$param-rnd-min"/>
...
<xsl:template match="de-term[@context='formula']" mode="runestone-fitb">
    <xsl:param name="term" />
    <xsl:text>Compute("</xsl:text>
    <xsl:call-template name="mustache-names-webwork" >
        <xsl:with-param name="mustache-string" select="." />
