...
<xsl:import href="./dynamic-common.xsl" />


<xsl:template match="setup" mode="runestone-fitb">
  <xsl:if match="de-parameter[@random]">
    <xsl:text>v._context = new MathContext(rand());&xa;</xsl:text>
  </xsl:if>
  <xsl:apply-templates match="*" mode="runestone-fitb" />
</xsl:template>
...
<xsl:template match="de-parameter" mode="runestone-setup">
    <xsl:text>v.</xsl:text><xsl:value-of select="@name"/><xsl:text> = v._context.AddParameter('</xsl:text>
    <xsl:value-of select="@name"/><xsl:text>', </xsl:text>
    <xsl:choose>
        <xsl:when test="@random='discrete'">
            <xsl:call-template name="random-parameter-discrete" >
              <xsl:with-param name="mode">runestone</xsl:with-param>
            </xsl:call-template>
        </xsl:when>
        ...
        <xsl:otherwise>
            <xsl:call-template name="mustache-names-runestone" >
...
<xsl:template match="de-term[@context='formula']" mode="runestone-setup">
    ...
    <xsl:text> = Formula(v._context, '</xsl:text><xsl:value-of select="@name"/><xsl:text>', </xsl:text>
    <xsl:call-template name="mustache-names-runestone" >
        <xsl:with-param name="mustache-string" select="." />
    </xsl:call-template>
    <xsl:text>);&#10;</xsl:text>
...
<xsl:template match="de-term[@context='substitution']" mode="runestone-setup">
    ...
    <xsl:text>v.</xsl:text><xsl:value-of select="@name"/>
    <xsl:text> = Composition(v._context, </xsl:text><xsl:value-of select="@name"/><xsl:text>, </xsl:text>
    <xsl:apply-templates select="." mode="rs-math" />
    <xsl:text>);&#10;</xsl:text>
</xsl:template>

<!-- General creation of formula object using substitution. -->
<xsl:template match="de-term[@context='substitution']" mode="rs-math">
    <xsl:choose>
        ...
        <xsl:otherwise>
            <xsl:text>Formula(v._context, "", "</xsl:text>
            <xsl:call-template name="mustache-names-runestone" >
